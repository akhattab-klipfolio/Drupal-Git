...
(function($) {
	...
	function trackYouTube() {
		...
		$('iframe').each(function(i) {
            ...
            // otherwise no protocol; added for Klipfolio
            else if (iframeSrc.substr(0, 24) == "//www.youtube.com/embed/") {
                baseUrlLength = 24;
                isYouTubeVideo = true;
            }
...
function onPlayerStateChange(event) {
	...
	if (event.data == YT.PlayerState.PLAYING) {
		// _gaq.push(['_trackEvent', 'Videos', 'Play', videoArray[videoIndex].id ]);  // using IDs for Klipfolio
		ga('send', 'event', 'Videos', 'Play', videoArray[videoIndex].id );
	}
	// if the video ends, send the event
	if (event.data == YT.PlayerState.ENDED) {
		// _gaq.push(['_trackEvent', 'Videos', 'Ended', videoArray[videoIndex].id ]); // using IDs for Klipfolio
		ga('send', 'event', 'Videos', 'Ended' , videoArray[videoIndex].id );
	}
}
